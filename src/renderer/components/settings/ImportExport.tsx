...
export default function ImportExport(): JSX.Element {
  ...
  const [steamResult, setSteamResult] = useState<SteamImportResult | null>(
    null,
  );
  ...
  async function handleSteamImport(): Promise<void> {
    ...
    setSteamResult(null);
    const result = await api.importSteamData(
      steamApiKey.trim(),
      steamId.trim(),
    );
  ...
  return (
    ...
    >
      ...
      {importResult && (
        <div className="import-result">
          <div className="import-result__stat">
            <span>Apps added</span>
            <span>{importResult.appsAdded}</span>
          </div>
          <div className="import-result__stat">
            <span>Apps updated</span>
            <span>{importResult.appsUpdated}</span>
          </div>
          <div className="import-result__stat">
            <span>Sessions imported</span>
            <span>{importResult.sessionsAdded}</span>
          </div>
          <div className="import-result__stat">
            <span>Duplicates skipped</span>
            <span>{importResult.duplicates}</span>
          </div>
      ...
      >
        ...
        {steamOpen && (
          ...
          >
            ...
            >
              Imports your Steam library as a one-time historical snapshot. Each
              game gets a single session representing its total playtime. Future
              play sessions will be tracked separately.
            </p>
            ...
            >
              ...
              >
                Find yours at store.steampowered.com/account (shown as "Steam
                ID")
              </span>
            </div>
            ...
            <button
              ...
              disabled={
                steamImporting || !steamApiKey.trim() || !steamId.trim()
              }
            ...
            {steamResult && (
              <div className="import-result" style={{ marginTop: 0 }}>
                <div className="import-result__stat">
                  ...
                  <span>{steamResult.gamesImported}</span>
                </div>
                <div className="import-result__stat">
                  <span>Sessions added</span>
                  <span>{steamResult.sessionsAdded}</span>
                </div>
                <div className="import-result__stat">
                  <span>Already imported</span>
                  <span>{steamResult.duplicates}</span>
                </div>
      ...
      <div className="settings-card danger-zone" style={{ marginTop: 8 }}>
        ...
        >
          This permanently deletes all session data. App list and settings are
          kept.
        </p>
        <button
        ...
        >
          Wipes everything — sessions, apps, groups, and settings — and resets
          to factory defaults.
        </p>
        <button
          className="btn btn--danger"
