...
export default function AppHeatmap({ appId, isGroup = false }: Props): JSX.Element {
  ...
  useEffect(() => {
    ...
    // Use getAppSessionRange with 'day' grouping to get per-day totals for this app
    api.getAppSessionRange(appId, oneYearAgo, now, 'day', isGroup)
      .then((result) => {
        const m = new Map<string, number>()
        // Convert chart points (which are day-based) to the map
  ...
  // Only show heatmap if there's any activity
  if (maxMs === 0) {
    return null as any
  }
  ...
  return (
    <div className="app-heatmap">
      <div className="app-heatmap__title">Activity History</div>
      <div className="app-heatmap__grid">
        <div className="heatmap-months">
          <div style={{ width: 18 }} />
          {weeks.map((_, wi) => {
            const label = monthLabelCols.find((m) => m.col === wi)
            return (
              <div key={wi} className="heatmap-month-cell app-heatmap__month">
                {label ? label.label : ''}
              </div>
            )
          })}
        ...
        <div style={{ display: 'flex', gap: 2 }}>
          <div className="heatmap-days">
            {DAY_LABELS.map((d, i) => (
              <div key={i} className="heatmap-day-label app-heatmap__day">{i % 2 === 1 ? d : ''}</div>
            ))}
          </div>
      ...
      {tooltip && (
        <div
          className="heatmap-tooltip app-heatmap__tooltip"
      ...
      <div className="heatmap-legend app-heatmap__legend">
